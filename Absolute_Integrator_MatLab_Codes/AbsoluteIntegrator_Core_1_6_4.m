...
if floor(now) > 735873 && strcmp(licenseStatus,'Online - valid') == 0 % Date should match here and in warning string below.
    ...
    if strcmp(licenseStatus,'Online - valid') == 0
        expiration_string = datestr(735873) ; % Date should match here and in logic check above.
...
%                         ylabel('Relative Sensitivity','FontSize',34,'FontWeight','bold')
    ...
    if size(input_file,3) > 1
        warning('Line added at 428 to average over multiple frames.')
        input_file = mean(input_file,3) ;
    end
    if loadSimulation == 1
    ...
    h2=waitbar(0,{'Now Determining Integration Ranges'},'Name',version_string);
    ...
    for i = 1:size(search_record,1)
        ...
        if (i/10) == round(i/10)
            ...
            waitbar_progress_text1 = horzcat ( 'Now Determining Integration Ranges.' ) ;
...
%             clear backgroundImage
    ...
    for point = 1:total_atoms
        ...
        integratedIntensity(point) = sum(sum(currentFeature(currentFeature ~= 0))) ;
        if subtractBackground == 1 && fitLocalBackground == 1
            ...
            backgroundIntensity(point) = sum(sum(currentFeature(currentFeature ~= 0))) ;
        end
    ...
    % These integrted intensitie are then scaled to give cross-sections:
...
%     [nelements,xcenters] = hist(integratedIntensity,total_atoms/16) ;
    ...
    xlabel('Scattering Cross Sections (Mb)')
    ylabel('Histogram Counts')
    % xlim([0 1.5e8])
